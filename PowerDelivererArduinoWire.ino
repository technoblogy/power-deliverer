/* Power Deliverer - Arduino Wire Version - see http://www.technoblogy.com/show?3Y8J

   David Johnson-Davies - www.technoblogy.com - 23rd August 2022
...
void PlotVal (uint16_t value, int line, int column, boolean volts) {
  boolean dig = false;
  ...
  PlotChar(Space, line, column); column = column + Scale;
  PlotChar(volts?Volts:Amps, line, column); column = column + Scale;
}
...
void PlotCursor (uint8_t line) {
  ...
  Wire.write(command);
  // Set column address range
  Wire.write(0x21); Wire.write(125); Wire.write(127);
  // Set page address range
  Wire.write(0x22); Wire.write(0); Wire.write(3);
  Wire.endTransmission();
  Wire.beginTransmission(OLEDAddress);
...
void InitSTUSB () {
  ...
  Wire.beginTransmission(STUSBAddress);
  Wire.write(PORT_STATUS);
  Wire.endTransmission();
  Wire.requestFrom(STUSBAddress, 0x16-0x0D+1);
  for (int i=0; i<0x16-0x0D+1; i++) Wire.read();

  // Only interested in PRT_STATUS_AL alert
  Wire.beginTransmission(STUSBAddress);
  Wire.write(ALERT_STATUS_MASK);
  Wire.write(~PRT_STATUS_AL);
  Wire.endTransmission();
}
...
uint8_t ReadRegister (uint8_t reg) {
  ...
  Wire.write(reg);
  Wire.endTransmission();
  Wire.requestFrom(STUSBAddress, 1);
  return Wire.read();
}

void SendSoftReset () {
  Wire.beginTransmission(STUSBAddress);
  Wire.write(TX_HEADER_LOW);
  Wire.write(0x0D); // Soft reset
  Wire.endTransmission();
  Wire.beginTransmission(STUSBAddress);
  Wire.write(PD_COMMAND_CTRL);
  Wire.write(0x26); // Send command
  Wire.endTransmission();
}
...
void ReadHeader () {
  Wire.beginTransmission(STUSBAddress);
  Wire.write(RX_HEADER);
  Wire.endTransmission(false);
  Wire.requestFrom(STUSBAddress, 2);
  PDHeader.bytes[0] = Wire.read(); PDHeader.bytes[1] = Wire.read();
}

// Read the PD objects
void ReadObjects () {
  Wire.beginTransmission(STUSBAddress);
  Wire.write(RX_DATA_OBJS);
  Wire.endTransmission(false);
  Wire.requestFrom(STUSBAddress, numObjects*4);
  for (int obj=0; obj<numObjects; obj++) {
    for (int i=0; i<4; i++) PDObject[obj].bytes[i] = Wire.read();
  }
}
...
void SelectPDO (uint8_t object) {
  uint8_t index = 1;
  Wire.beginTransmission(STUSBAddress);
  Wire.write(DPM_SNK_PDOS + 4*index);
  for (int i=0; i<4; i++) Wire.write(PDObject[object].bytes[i]);
  Wire.endTransmission();
}
...
void UpdatePDONumber (uint8_t Number_PDO) {
  if (Number_PDO >= 1 && Number_PDO <= 3) {
    Wire.beginTransmission(STUSBAddress);
    Wire.write(DPM_PDO_NUMB);
    Wire.write(Number_PDO);
    Wire.endTransmission();
  }
}
...
void DisplayPDObjects () {
  for (int obj = 0; obj<Objects; obj++) {
    ...
    PlotChar(Ket, obj, 2);
    PlotVal(PDObject[obj].voltage, obj, 5, true);
    PlotVal(PDObject[obj].current, obj, 13, false);
  }
}
...
const int Up = 5, Down = 6, Select = 7;
volatile boolean SelectPressed;
volatile uint8_t Cursor = 0;

void ButtonsOn () {
  // PA5 (Up), PA6 (Down), and PA7 (Select) inputs with pullups
...
void SelectVoltage () {
  ...
  ClearBuf();
  Scale = 2;
  PlotVal(PDObject[Cursor].voltage, 0, 5, true);
  PlotVal(PDObject[Cursor].current, 3, 7, false);
  UpdateScreen();
  WaitSelect();                                       // Wait for Select button
}
...
void setup() {
  delay(100);
  Wire.begin();
  Wire.setClock(400000);  // Not reliable on default speed
  InitDisplay();
  UpdateScreen();
  DisplayOn();
